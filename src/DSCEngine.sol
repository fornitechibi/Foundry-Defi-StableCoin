...
import {OrcaleLib} from "./libraries/OracleLib.sol";
 ...
 *
 * It is similar to DAI if DAI had no fees, no governance and was only backed by WETH and WBTC.
 *
...
contract DSCEngine is ReentrancyGuard {
    ...
    uint256 private constant LIQUIDATION_THERSHOLD = 50;
    uint256 private constant LIQUIDATION_PRECISION = 100;
    uint256 private constant MIN_HEALTH_FACTOR = 1e18;
    uint256 private constant LIQUIDATION_BONUS = 10; // this means 10% bonus

    mapping(address token => address priceFeed) private s_priceFeeds;
    mapping(address user => mapping(address token => uint256 amount)) private s_collateralDeposisted;
    ...
    event CollateralDeposisted(address indexed user, address indexed token, uint256 amount);
     ...
     * @param amountCollateral The amount to collateral to deposit
     * @param amountDscToMint The amount of decentralized stablecoin to deposit
    ...
    /**
     * @notice This function uses CEI => (Checks, Effects and Interactions) method
     * @param tokenCollaterallAddress The address of token to deposit as collateral.
    ...
    function depositCollateral(address tokenCollaterallAddress, uint256 amountCollateral)
        ...
        nonReentrant
    {
        s_collateralDeposisted[msg.sender][tokenCollaterallAddress] += amountCollateral;
        emit CollateralDeposisted(msg.sender, tokenCollaterallAddress, amountCollateral);
     ...
     * @notice They must have more collateral than the minimum thershold
     */
     ...
     * @param debtToCover The amount of DSC you want to burn to imporve users health factor
     ...
     * @notice A know bug would be if the protocol were 100% or less collateralized, then we would't be able to incentive the liquidators.
     * For example, if the price of the collateral plummeted before anyone could be liquidated.
     * Follows CEI: checks, effects, and interaction.
     */
        ...
        returns (uint256)
    {
        ...
        uint256 collateralAdjustedForThreshold = (CollateralValueInUsd * LIQUIDATION_THERSHOLD) / LIQUIDATION_PRECISION;
        return (collateralAdjustedForThreshold * PRECISION) / totalDscMinted;
    }

    // 1. Check health factor (do they enough collateral?)
    ...
    function _redeemCollateral(address from, address to, address tokenCollateralAddress, uint256 amountCollateral)
        private
    {
        s_collateralDeposisted[from][tokenCollateralAddress] -= amountCollateral;
    ...
    function getAccountCollateralValue(address user) public view returns (uint256 totalCollateralValueinUsd) {
        ...
        for (uint256 i = 0; i < s_collateralTokens.length; i++) {
            ...
            uint256 amount = s_collateralDeposisted[user][token];
    ...
    function getCollateralBalanceOfUser(address user, address token) external view returns (uint256) {
        return s_collateralDeposisted[user][token];
    }
